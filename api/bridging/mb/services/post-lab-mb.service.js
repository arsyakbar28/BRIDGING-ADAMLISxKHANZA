/**
 * POST Lab MB Service
 * Business logic for posting lab results (MB)
 */
...
async function postLabResults(noorder, labData) {
    ...
    try {
        ...
        // Get request info from permintaan_labmb
        const requestInfo = await postLabRepository.getLabRequestInfo(conn, noorder);
        if (!requestInfo) {
            await conn.rollback();
        ...
        const orderStatus = String(requestInfo.status || '').toLowerCase() === 'ranap' ? 'Ranap' : 'Ralan';
        ...
        const response = {
            success: true,
            message: `MB lab results posted successfully for noorder: ${noorder}`,
            summary: {
                noorder, 
                no_rawat, 
